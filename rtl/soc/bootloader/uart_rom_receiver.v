            ...
            if (rx_ready && !done) begin
                ...
                if (byte_index == 3) begin
                    ...
                    if (word_index == firmware_size-1) begin
                        $display("[BOOT] Firmware carregado 100%% (%0d/%0d words)", word_index + 1, firmware_size);
                        done <= 1;
                    end
                    else begin
                        word_index <= word_index + 1;
                        if ((word_index % 10) == 0) begin
                            $display("[BOOT] Progresso: %0d / %0d words (%0d%%)", word_index, firmware_size, (word_index * 100) / firmware_size);
                        end
                    end
                end
            end
